...
from euler.logger import JsonFormatter, logger
...
class TestLogger(unittest.TestCase):
    def test_logger_is_configured(self):
        ...
        self.assertEqual(logger.name, "euler")
    ...
    def test_logger_handles_no_handlers(self):
        ...
        with self.assertLogs('euler', level='WARNING') as captured:
            logger.warning("This is a test warning without handlers.")
        logger.handlers = original_handlers
        logger.setLevel('ERROR')

        # Verify the logger captures the warning message
        self.assertIn("WARNING:euler:This is a test warning without handlers.", captured.output)

    @patch('euler.logger.logger', new_callable=MagicMock)
    def test_logger_already_initialized(self, mock_logger):
        ...
        # Patch the `logger` directly in `euler.logger`
        with patch('euler.logger.logger', mock_logger_already_initialized):
            # Re-import `logger` from the `euler.logger` module
            from euler.logger import logger as reimported_logger
...
class TestJsonFormatter(unittest.TestCase):
    def test_json_formatter_format_with_exception(self):
        ...
        try:
            ...
            self.assertEqual(parsed_log["name"], "euler.test")
