...
from euler.maths.integer_partitions import (
    IntegerPartitionError,
    get_partitions,
    get_prime_partitions,
    main,
    num_integer_partitions,
    num_partitions,
    num_prime_integer_partitions,
)
...
class TestIntegerPartitions(unittest.TestCase):
    def test_num_partitions_known_values(self):
        ...
        for n, expected in known.items():
            with self.subTest(n=n):
                self.assertEqual(num_partitions(n), expected)
    ...
    def test_get_partitions_examples(self):
        ...
        parts_5 = get_partitions(number=5, slots=5)
        self.assertEqual(len(parts_5), num_partitions(5))
        for p in parts_5:
    ...
    def test_get_partitions_errors_and_boundaries(self):
        ...
        with self.assertRaises(OverflowError) as cm:
            get_partitions(number=6, slots=6, safe_limit=5)
        self.assertEqual(str(cm.exception), 'number must be less than safe_limit=5')

        # Negative / number < slots errors
        with self.assertRaises(IntegerPartitionError) as cm:
            get_partitions(number=-1, slots=0)
        self.assertEqual(str(cm.exception), 'number and slots must be non-negative')

        with self.assertRaises(IntegerPartitionError) as cm:
            get_partitions(number=3, slots=4)
        self.assertEqual(str(cm.exception), 'number must be greater than or equal to slots')
    ...
    def test_num_prime_integer_partitions_examples_and_edges(self):
        ...
        self.assertEqual(num_prime_integer_partitions(number=10, slots=10), 5)
        # With slots=5, primes greater than 5 are excluded; valid: [2,2,2,2,2], [3,3,2,2], [5,3,2], [5,5]
        self.assertEqual(num_prime_integer_partitions(number=10, slots=5), 4)
        # Additional small checks
        self.assertEqual(num_prime_integer_partitions(number=5, slots=5), 2)  # [2,3], [5]
        # Edges
        self.assertEqual(num_prime_integer_partitions(number=0, slots=10), 1)
        self.assertEqual(num_prime_integer_partitions(number=10, slots=1), 0)
        with self.assertRaises(IntegerPartitionError) as cm:
            num_prime_integer_partitions(number=-1, slots=10)
        self.assertEqual(str(cm.exception), 'number and slots must be non-negative')
    ...
    def test_get_prime_partitions_examples_and_boundaries(self):
        ...
        self.assertEqual(get_prime_partitions(number=10, slots=1), [])

    def test_get_prime_partitions_errors(self):
        # Overflow error with exact message
        with self.assertRaises(OverflowError) as cm:
            get_prime_partitions(number=6, slots=6, safe_limit=5)
        self.assertEqual(str(cm.exception), 'number must be less than safe_limit=5')

        # Negative error
        with self.assertRaises(IntegerPartitionError) as cm:
            get_prime_partitions(number=-2, slots=5)
        self.assertEqual(str(cm.exception), 'number and slots must be non-negative')
