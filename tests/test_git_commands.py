...
class CommitRegeneratedTest(_GitCommandCase):
    """`commit_regenerated`: the commit each `update-*` verb makes for its own output.

    What matters here is **scope** — an automatic commit has nobody reviewing it, so it must
    stage the paths that verb writes and nothing beside them — and the **subject type**,
    because `scripts/version/release.sh` reads the Conventional Commit types since the last
    tag to pick the SemVer bump.
    """

    def setUp(self) -> None:
        super().setUp()
        self.dirty: str = ' M docs/user-guide.md'
        git.run = lambda *a, **k: MagicMock(returncode=0, stdout=self.dirty)  # type: ignore[assignment]

    def test_every_update_verb_has_a_path_set(self) -> None:
    ...
    def test_clean_paths_commit_nothing(self) -> None:
        self.dirty = ''
        self.assertEqual(git.commit_regenerated('update-docs', ('quip',), ['docs/user-guide.md']), 0)
        self.assertEqual(self.cmdlines, [])

    def test_writing_nothing_commits_nothing(self) -> None:
        """An empty *written* is the whole answer — there is deliberately no fallback to
        `GENERATED_PATHS[verb]`, which for `update-docs` is `docs/` and would sweep up every
        hand-written guide dirty at that moment."""
        self.assertEqual(git.commit_regenerated('update-docs', ('quip',), []), 0)
        self.assertEqual(self.cmdlines, [])

    def test_the_subject_is_a_chore_so_the_release_bump_stays_a_patch(self) -> None:
        git.commit_regenerated('update-usd-rate', ('the euro moved',), ['solver/config/values.conf'])
        self.assertIn("--message 'chore(update-usd-rate): the euro moved", self.cmdlines[0])

    def test_the_body_carries_the_facts(self) -> None:
        git.commit_regenerated('update-docs', ('quip',), ['docs/user-guide.md'],
                               ['README.md: package-layout'])
        self.assertIn('Regenerated by `update-docs`', self.cmdlines[0])
        self.assertIn('README.md: package-layout', self.cmdlines[0])
    ...
    def test_a_verb_stages_only_its_own_output(self) -> None:
        ...
        git.commit_regenerated('update-models', ('quip',), ['solver/ai/models.py'])
        self.assertIn('solver/ai/models.py', self.cmdlines[0])
        self.assertNotIn('solver/config/values.conf', self.cmdlines[0])

    def test_only_the_files_written_are_committed_not_the_whole_scope(self) -> None:
        """The guarantee the path table alone cannot give. `update-docs` owns `docs/`, which
        also holds the hand-written guides — so a run that regenerated one file must commit
        that file and leave a guide someone is drafting exactly where it is."""
        git.commit_regenerated('update-docs', ('quip',), ['docs/commands-index.md'])
        self.assertIn('docs/commands-index.md', self.cmdlines[0])
        self.assertNotIn('docs/web-server-guide.md', self.cmdlines[0])
        self.assertNotIn(" docs/ ", self.cmdlines[0])

    def test_the_commit_is_pathspecd_so_staged_work_survives(self) -> None:
        """Scoping the `add` is not enough: a bare `git commit` writes the whole index, so a
        regeneration run while the committer had staged work of their own carried that work
        off under a `chore(update-*)` subject. The commit must carry the pathspecs too."""
        git.commit_regenerated('update-docs', ('quip',), ['docs/user-guide.md'])
        add, _, commit = self.cmdlines[0].partition('&&')
        self.assertIn('docs/user-guide.md', add)
        self.assertIn('git commit', commit)
        self.assertIn('-- ', commit)
        self.assertIn('docs/user-guide.md', commit.split('-- ', 1)[1])

    def test_a_path_outside_the_verbs_scope_is_dropped(self) -> None:
        """`GENERATED_PATHS` stays the envelope: a verb reporting a write beyond it has a bug,
        and the commit refuses to carry the file rather than quietly widening itself."""
        git.commit_regenerated('update-models', ('quip',),
                               ['solver/ai/models.py', 'solver/core/git.py'])
        self.assertIn('solver/ai/models.py', self.cmdlines[0])
        self.assertNotIn('solver/core/git.py', self.cmdlines[0])

    def test_a_path_naming_nothing_is_dropped(self) -> None:
        """A reported path that is neither on disk nor tracked is always a bug in the verb —
        and `git commit -- <spec>` fails outright on a pathspec matching nothing, so leaving
        it in would turn every one of that verb's commits into a failure."""
        git.run = lambda *a, **k: MagicMock(  # type: ignore[assignment]
            returncode=0 if a and 'status' in a[0] else 1, stdout=self.dirty)
        git.commit_regenerated('update-docs', ('quip',),
                               ['docs/user-guide.md', 'docs/no-such-file.md'])
        self.assertIn('docs/user-guide.md', self.cmdlines[0])
        self.assertNotIn('no-such-file', self.cmdlines[0])

    def test_a_failed_commit_is_reported_not_swallowed(self) -> None:
        self.rcs = [1]
        self.assertEqual(git.commit_regenerated('update-docs', ('quip',), ['docs/user-guide.md']), 1)
