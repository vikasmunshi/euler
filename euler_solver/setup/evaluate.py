...
from euler_solver.utils.human_readable_time import human_readable_seconds
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                evaluation_result.failed_problems_list.append(str(euler_problem))
...
def evaluate_and_get_evaluation_result(euler_problem: int, *,
    ...
    if (registry is None or len(registry.solutions) == 0 or len(registry.test_cases) == 0
            or sum(len(s.test_cases) for s in registry.solutions) == 0):
        return EvaluationResult()
    evaluation_result: EvaluationResult = EvaluationResult(total_problems=1)
    if func_def_len is None:
        func_def_len = max(len(tc.execution_time_key(s)) for s in registry.solutions for tc in s.test_cases) + 1
    for solution, test_case in ((s, tc) for s in registry.solutions for tc in s.test_cases):
        ...
        if mode == 'list':
            ...
            print(format_msg(status='info',
                             ...
                             execution_time=registry.test_case_answers.get(et_key, ''), ))
            continue
...
def evaluate_catch_exceptions(solution: Solution, test_case: TestCase,
    ...
    try:
        ...
        py_file_path: Path = get_module_path(solution.euler_problem)
        print(f'{Color.RED}Solution for problem {solution.euler_problem} not implemented: {Color.RESET}'
...
def print_and_tabulate(*,
    ...
    if result is None:
        ...
        evaluation_result.failed_problems_list.append(f'{euler_problem}:{func_def.strip()!s}')
    elif mode == 'record':
