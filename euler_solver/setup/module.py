...
from euler_solver.setup.paths import get_module_fqdn, get_module_path
from euler_solver.setup.patterns import answer_re, stubber_re
from euler_solver.setup.register import get_registry
...
def setup_solution_template(start_number: int, end_number: int, *,
                            force_update_doc: bool = False,
                            max_async: int) -> None:
    with Runner() as runner:
        runner.run(create_py_and_pyi_idempotent_for_range(start_number, end_number,
                                                          force_update_doc=force_update_doc,
                                                          max_async=max_async))


async def create_py_and_pyi_idempotent_for_range(start_number: int, end_number: int, *,
                                                 force_update_doc: bool = False,
                                                 max_async: int) -> None:
...
async def _create_with_semaphore(euler_problem: int, *, force_update_doc: bool, sem: Semaphore,
    ...
    async with sem:
        try:
            await create_py_and_pyi_idempotent(euler_problem, force_update_doc=force_update_doc)
...
async def create_py_and_pyi_idempotent(euler_problem: int, *, force_update_doc: bool = False) -> None:
    ...
    pyi_file_path_exists: bool = pyi_file_path.exists()
    ...
    try:
        ...
        logger.error({'action': 'Final Module Verification Failed', 'euler_problem': euler_problem,
                      'error': repr(e)})
        raise e
    if not (py_file_path_exists and pyi_file_path_exists):
        with FileLock(py_file_path, 'read') as f:
            source_code = f.read()
        stub_code: str = stubber_re.sub(r'\1 ...\n\n', answer_re.sub('Answer: ...', source_code))
        pyi_file_path = py_file_path.with_suffix('.pyi')
        with FileLock(pyi_file_path, 'write') as f:
            f.write(stub_code)
        logger.info({'action': 'Solution Template Created', 'euler_problem': euler_problem, 'file': pyi_file_path})
    ...
    created: str = f'{Color.ORANGE}⬆'
    exists: str = f'{Color.BLUE}🗋'
    print(f'{exists if py_file_path_exists and pyi_file_path_exists and json_file_path_exists else created} '
          f'{euler_problem} '
          f'{exists if py_file_path_exists else created} py '
          f'{exists if pyi_file_path_exists else created} pyi '
          f'{exists if json_file_path_exists else created} json '
          f'{Color.RESET}\n'
          f'file://{py_file_path}\n'
          f'file://{pyi_file_path}\n'
