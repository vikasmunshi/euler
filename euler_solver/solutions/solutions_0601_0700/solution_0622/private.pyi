...
Solution Approach:
    Model the riffle shuffle as a permutation on the deck indices. Use number theory to find the
    order of this permutation efficiently, relating it to modular arithmetic properties.
    Calculate s(n) by determining the order of the shuffle permutation for each even n.
    Employ efficient arithmetic and factorization techniques to handle large n values.
    The problem requires summation over all n with s(n) = 60, so optimize by pruning via order
    divisibility properties and fast cycle detection.
...
test_cases: list[dict[str, Any]] = [
    {'category': 'dev', 'input': {'s_target': 8, 'max_limit': 100}},
    {'category': 'main', 'input': {'s_target': 60}},
]


@register_solution(euler_problem=euler_problem, max_test_case_index=None)
def solve_riffle_shuffles_p0622_s0(*, s_target: int, max_limit: int | None = None) -> int: ...
