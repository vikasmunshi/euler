...
parser.add_argument('--mode',
                    ...
                    help='Evaluation mode (default: evaluate) evaluate: '
                         ...
                         'evaluate-dev: evaluate the solutions against dev test cases;'
...
parser.add_argument('--setup',
                    action='store_true',
                    default=False,
                    help='Create solution template for new problem (max parallel is capped at 4) (default: False)')
...
def main() -> int:
    ...
    if args.setup:
        from euler_solver.setup.module import setup_solution_template
        max_async: int = min(8, args.max_workers or 8)
        setup_solution_template(start_number, end_number, force_update_doc=False, max_async=max_async)
        return 0
    ...
    match args.mode:
        case 'evaluate':
            mode = 'evaluate'
            set_evaluation_options(eval_dev=False, eval_main=True, eval_extra=False, ignore_slices=False)
        case 'evaluate-all':
            mode = 'evaluate'
            set_evaluation_options(eval_dev=True, eval_main=True, eval_extra=True, ignore_slices=False)
        case 'evaluate-dev':
            mode = 'evaluate'
            set_evaluation_options(eval_dev=True, eval_main=False, eval_extra=False, ignore_slices=False)
        case 'list':
            mode = 'list'
            max_workers = 1
            set_evaluation_options(eval_dev=True, eval_main=True, eval_extra=True, ignore_slices=True)
        case 'record':
            mode = 'record'
            set_evaluation_options(eval_dev=False, eval_main=True, eval_extra=False, ignore_slices=False)
        case 'record-all':
