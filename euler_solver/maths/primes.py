...
from euler_solver.c_libs.primes import fast_is_prime, primes_sundaram_sieve
...
def prime_factorization(n: int) -> tuple[Factor, ...]:
    ...
    for prime in primes_sundaram_sieve(max_num=int(n ** 0.5) + 1):
        if prime > n:
            break
        exponent = 0
        while n % prime == 0:
    ...
    while factor <= int(num ** 0.5):
        for gap in ([1, 1, 2, 2, 4] if factor < 11 else [2, 4, 2, 4, 6, 2, 6, 4]):
            ...
            while num % factor == 0:
                num //= factor
                if is_new_factor:
                    number_factors, is_new_factor = number_factors + 1, False
    return number_factors + (0 if num == 1 else 1)
