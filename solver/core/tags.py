...
#: The article statuses, in writing order — a page climbs missing → draft → final.
STATUSES = ('missing', 'draft', 'final')
...
def _regen_articles(central: dict[str, Any], write: bool) -> list[str]:
    """Refill each article's generated problem list from its declared tags, and stamp a
    ``draft`` status on any article that declares none. Returns issues."""
...
def _build_index(central: dict[str, Any]) -> dict[str, Any]:
    """The article index (``topics/articles.json``): one row per writable topic.

    Every tag in the vocabulary is a topic that *could* be written (status ``missing``
    until its page exists); every page on disk overrides its row with what the file
    actually says — its title, its declared tags and its status. Curated pages that are
    not a tag (``number-theory/primes``) join on the same footing.
    """
    by_slug = {t['slug']: t for t in central['tags']}
    rows: dict[str, dict[str, Any]] = {}
    for tag in central['tags']:
        path = f"{tag['facet']}/{tag['slug']}"
        rows[path] = {'path': path, 'title': tag['name'], 'status': 'missing',
                      'tags': [tag['slug']], 'problems': _problem_count(tag)}
    for file in _iter_articles():
        text = file.read_text()
        path = file.relative_to(config.topics_dir).with_suffix('').as_posix()
        declared = _article_tags(text)
        rows[path] = {'path': path,
                      'title': _article_title(text, rows.get(path, {}).get('title', file.stem)),
                      'status': article_status(text), 'tags': declared,
                      'problems': len({r.split('_')[0] for slug in declared
