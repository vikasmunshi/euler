...
#: The generated reference list is kept inside a single HTML comment — maintained for reference
#: (grep, git) but not rendered. update-tags refills its body from the declared tags' central legs,
#: at the same granularity the legs use: ``pNNNN`` for domain/takeaway, ``pNNNN_sN`` for technique.
_GEN_MARKER = 'refs (generated by update-tags)'
...
def _create_article(path: Path, tag: dict[str, Any], refs: list[str]) -> None:
    ...
    body = get_template(Templates.ARTICLE).substitute(
        slug=tag['slug'], title=tag['name'], gen_marker=_GEN_MARKER,
...
def _regen_articles(central: dict[str, Any], write: bool) -> list[str]:
    """Create a page for every tag that lacks one, then refill each page's generated ref list.
    ...
    if write:
        for tag in central['tags']:
            ...
            if not path.exists():
                _create_article(path, tag, _article_refs([tag['slug']], by_slug))
    for path in _iter_articles():
        text = path.read_text()
        ...
        replacement = f'<!-- {_GEN_MARKER}\n' + '\n'.join(_article_refs(declared, by_slug)) + '\n-->'
