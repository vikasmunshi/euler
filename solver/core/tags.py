...
def format_vocabulary(facets: tuple[str, ...] = FACETS) -> str:
    ...
    When more than one facet is listed, each entry repeats its facet inline
    (`- slug [technique] — name`). Section grouping alone is not enough: a generator
    recalling a slug loses which
...
def tags(details: bool = False) -> int:
    ...
    Args:
        details: Add, per facet, a histogram of how many problems each tag maps to — min /
            median / max, then binned counts. Defaults to False, which prints just the
            number of tags in each facet.
    """
...
def topics(problem: Problem) -> int:
    ...
    Args:
        problem: [problem] The problem whose tags to list.
    """
...
def update_tags(check: bool = False) -> int:
    ...
    Args:
        check: Write nothing: report unknown slugs, facet violations, unpromoted proposals,
            ...
            Defaults to False, which reconciles the graph in place.
    """
