...
3. Read the target file if it exists — it is usually a **skeleton**: a `<!-- tags: [...] -->`
   comment on line 1, a `<!-- status: draft -->` comment under it, an `# H1` title, a `_TODO: …_`
   stub, and a generated `<!-- problems (generated by update-tags) … -->` block (the reference list
   of problem numbers, kept inside an HTML comment so it is **not rendered**). You are replacing the
   stub with real prose; **keep the tags comment and the problems comment untouched**. The status
   comment is yours to flip in Phase 4 — leave it at `draft` until the page is written.
...
page must open with its own `<!-- tags: [slug, …] -->` comment, carry a `<!-- status: draft -->`
comment under it, and end with an empty `<!-- problems (generated by update-tags)\n-->` comment.
...
2. **Refresh the problem list:** `solver "update-tags"` — it refills every article's generated
   problems block, reconciles the graph, and rewrites `topics/articles.json` (where your new status
   lands). Then `solver "update-tags --check"` must exit `0`; fix any issue it names (an unknown tag
   in your `<!-- tags: -->` comment, most likely) and re-run.
3. **Sanity-check:** `solver "topic <path>"` lists the tags and problems the page now resolves to.
4. **Commit the page with plain git.** Do **not** use `solver "commit"` — that command is
   ...
   If `git status` shows other files that `update-tags` touched (e.g. `topics/tags.json`,
   `topics/articles.json`), stage and include them in the same commit. Topic pages carry no solutions, so the message needs no
