...
- Per-user private key: `~/.ssh/id_solver` (X25519)
- `keys/keys.json`: pool of AES-256 file-encryption keys, each wrapped with the master key; one master-key entry per authorised user (wrapped with that user's X25519 public key)
- Decryption path: private key → unwrap master key → unwrap file-encryption key → decrypt file
