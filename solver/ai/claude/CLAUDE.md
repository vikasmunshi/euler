...
solver/
  ...
  ai/
    ...
    update_models.py   — The `update-models` command: refresh the `Model` enum and its pricing.
    update_usd_rate.py — The `update-usd-rate` command: refresh the USD→EUR reference rate.
...
- **`claude-solve` / `claude-blog`** (`solver/ai/skill.py`) launch Claude Code headless via a skill under `solver/ai/claude/skills/`, running at the repo root. `claude-solve` works a problem's solution files through the `claude-euler-solver` skill (`claude -p /claude-euler-solver <problem_number> <action>`; actions `solve` and `review`); `claude-blog` writes one topic article under `topics/` through the `claude-euler-blogger` skill (`claude -p /claude-euler-blogger <tag-or-topic>`). Both finish the same way — a local commit, and no further: pushing the branch and opening its pull request (`git-push`) is the collaborator's own step, taken once they have reviewed and amended the work. Both are held to the `docs/convention_*.md` guides (also injected into the `claude-api` prompts via `templates/engine.py`). Every skill in that directory is composed into the web front end's `/docs/ai` page.
