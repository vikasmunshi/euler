...
# -*- coding: utf-8 -*-
"""Web-shell service runtime configuration, read from the environment (DD-5/DD-13).
...
class WsConfig(NamedTuple):
    ...
    tcp_bind: str
    #: The profile this instance is *born* as (``EULER_PROFILE=%i``, DD-12/DD-13).
    ...
    #: child, whose redeemed ticket profile must match it (DD-13). Empty (dev)
    #: accepts any known profile.
    profile: str
    #: The auth service's public socket — shell-ticket minting (DD-9).
    auth_socket: str
    ...
    #: it (DD-14 hygiene; 0 disables). Enforcement lands in Phase 6 step 5.
    detached_ttl: int
