...
#: The machine-owned regions of a topic article. `tags` is the page's graph identity (what
#: update-tags reconciles against), `status` is draft/final, and `problems` is the generated,
#: rendered Problems section between its two sentinel comments. The in-browser editor lets a
#: maintainer rewrite the prose; it must not let a stray keystroke drop or mangle any of these,
#: or the page detaches from the tag graph. Defined here rather than imported from
#: `solver.core.tags` for the same reason `content.topic_status` is: that module drags in the
#: shell framework.
_ART_TAGS_RE = re.compile(r'<!--\s*tags:\s*\[.*?\]\s*-->', re.DOTALL)
_ART_STATUS_RE = re.compile(r'<!--\s*status:\s*\w+\s*-->', re.IGNORECASE)
_ART_PROBLEMS_RE = re.compile(r'<!--\s*problems \(generated by update-tags\)\s*-->.*?'
                              r'<!--\s*/problems\s*-->', re.DOTALL)
_ART_REGIONS = ((_ART_TAGS_RE, 'tags'), (_ART_PROBLEMS_RE, 'problems'), (_ART_STATUS_RE, 'status'))
...
def validate_article(submitted: str, original: str) -> Validated:
    ...
    for rx, what in _ART_REGIONS:
        want, got = _one(rx, original), _one(rx, submitted)
        if want is None:                      # the on-disk page is malformed; not the edit's fault
            continue
        if got is None:
            return _fail(f'the {what} block is machine-owned and must be kept exactly once — '
                         f'it is missing or duplicated in your edit')
        if got.strip() != want.strip():
            return _fail(f'the {what} block is maintained by update-tags and cannot be edited here')

    prose = submitted
    for rx, _ in _ART_REGIONS:
        prose = rx.sub('', prose)
    if '<!--' in prose:
        return _fail('remove the stray HTML comment — only the tags, status and problems blocks '
