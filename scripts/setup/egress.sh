...
# (the inbound edge); see docs/web-server-guide.md § Egress.
#
# Model:
...
#   - Runs as the dedicated `euler-proxy` user, in its own group — NOT in
...
# ── Service identity: euler-proxy is its own user/group, outside euler-web ──
...
# Squid forward proxy for euler egress.
#
...
# only. See docs/web-server-guide.md.
...
# see docs/web-server-guide.md § Egress. To use it in a shell:  set -a; . ${EGRESS_ENV}
