...
#
# Model (runs as the non-root euler-ddns user):
...
#
# Present when this runs from the checkout (the installer role). DELIBERATELY optional:
# `deploy` installs a copy of THIS file as ${DDNS_BIN}, where SCRIPT_DIR is /usr/local/bin
# and no helper sits beside it. That copy is the runtime updater, which reads only the
# scoped plaintext ${DDNS_ENV} and so needs no vault reader — see load_config. Sourcing
# unconditionally here killed the updater on startup (it is `set -e`) while the installer,
# running from the repo, still reported success.
if [ -r "${SCRIPT_DIR}/authoring_env.sh" ]; then
...
# operator running an install cannot read it and lands on the authoring env, while the
# euler-ddns runtime reads the scoped copy and never sees the operator's.
#
# The two sources take different paths on purpose. ddns.env is generated by
# deploy_ddns_env and is therefore ALWAYS plaintext, so it is sourced directly — that is
# what lets the deployed updater run without the authoring-env helper beside it. Only
# ~/.euler/env can be vault ciphertext and need the reader.
load_config() {
    if [ -r "${DDNS_ENV}" ]; then
        set -a
    ...
    if [ -z "${FQDN}" ]; then
        echo "Error: EULER_TLS_DOMAIN not set in ${DDNS_ENV} or ${ENV_FILE:-~/.euler/env}" >&2
        return 1
    fi
}
