...
#
# Usage:
#   ./upgrade_service.sh install                 → Install/setup upgrade service
#   ./upgrade_service.sh uninstall               → Remove upgrade service
...
#   - /var/refs/upgrade/last_upgrade_check       → Last run timestamp (644)
...
#   cat /var/refs/upgrade/last_upgrade_check → Check last run date
#
...
install_all() {
  ...
  install -d -m 0755 /var/log /usr/local/bin /var/reference_implementations/upgrade /run/upgrade
  touch "$LOG_FILE"
  chmod 0644 "$LOG_FILE"
...
uninstall_all() {
  ...
  rm -f /var/reference_implementations/upgrade/last_upgrade_check
...
show_help() {
  cat <<'HELP'
Usage: upgrade_service.sh [OPTION]

Install and configure a systemd-based automatic system upgrade service.

Options:
  install        Install and configure a systemd-based automatic system upgrade service.
  uninstall      Remove the upgrade service and all associated files
  --help, -h     Display this help message and exit

Examples:
  ./upgrade_service.sh install                  Install/setup upgrade service
  ./upgrade_service.sh uninstall                Remove upgrade service
  ./upgrade_service.sh --help                   Display this help message
  sudo DEBUG=1 systemctl start upgrade.service  Test with tracing
...
Files created:
  ...
  /usr/local/bin/status-jobs                    - Status check script
  /var/log/upgrade.log                          - Upgrade log file
  /var/lib/upgrade/last_upgrade_check           - Last run timestamp
  /run/upgrade/upgrade.lock                     - Runtime lock file
