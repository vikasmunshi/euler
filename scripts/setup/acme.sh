...
# credentials from the project env file keys/.env (the same file that holds
# ANTHROPIC_API_KEY) or the existing environment:
...
ENV_FILE="${PROJECT_ROOT}/keys/.env"            # DNS provider credentials (see header)
...
# provider's credentials for acme.sh, from keys/.env if present, else from the
