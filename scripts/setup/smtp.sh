#!/usr/bin/env bash
# Loopback SMTP relay (euler-smtp) — Phase 4, step 1 of the server redesign (DD-8)
# ================================================================================
#
...
# egress.sh / ddns.sh / firewall.sh; see docs/server-redesign.md (DD-8).
#
# Model:
...
#     the installer from keys/.env (the authoring source: SMTP_ADDRESS /
#     SMTP_APP_PASSWORD). euler-smtp never reads the full keys/.env.
#
...
# The loopback listener euler-auth submits to (EULER_SMTP_RELAY in auth.env, DD-6).
...
Usage: $0 [install|uninstall|upgrade|status|test|help]
  ...
  Authoring config in keys/.env: SMTP_ADDRESS + SMTP_APP_PASSWORD (Gmail app
...
# /etc/euler/smtp.env (readable by root/euler-smtp); falls back to the repo keys/.env
...
# Create the dedicated euler-smtp user (own group, nologin, outside euler-web) — DD-8.
ensure_smtp_user() {
...
# GENERATED by scripts/setup/smtp.sh — scoped runtime config for euler-smtp (DD-8).
# Authoring source: keys/.env. Read ONLY by the relay — no app service sees these.
