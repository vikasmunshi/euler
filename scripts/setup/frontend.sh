...
${DOMAIN} {
	...
	# then the session's own per-user instance proxies, else the holding page.
	handle {
		route {
			...
			# No (well-formed) slug → holding page.
			import maintenance
		}
	}

	# When an upstream reverse_proxy fails — e.g. a not-yet-provisioned or
	# deprovisioned user's socket — serve the same holding page instead of a bare
	# 502/504.
	handle_errors {
		import maintenance
	}
