...
# (Design decisions DD-1..DD-4; Phases 1 and 3).
#
# Topology through Phase 3:
#   - Caddy terminates TLS on :443, serves a Caddy-native health endpoint
#     (`/healthz` -> 200), the maintenance page's static assets (`/assets/*`), and a
#     static "under maintenance" holding page (HTTP 503) for every other request.
#     App-service upstreams (unix sockets under /run/euler) and the forward_auth gate
#     are added by later phases; the Caddyfile ships them as commented stubs, and a
#     handle_errors block reuses the same holding page when an upstream is down.
...
# Caddy configuration for the solver web edge (Phase 3).
#
# GENERATED by scripts/setup/frontend.sh — edits are overwritten on install/upgrade.
# Caddy terminates TLS and serves a health endpoint plus the static maintenance
# holding page (from ${WEB_CONTENT_DIR}). App-service upstreams (unix sockets under
# /run/euler) and the forward_auth gate are added by later phases (see the commented
# stubs below and docs/server-redesign.md). Until then — and, later, whenever an
# upstream is down — every request falls through to the maintenance page. The cert is
# issued out-of-band by acme.sh (DNS-01) into ${TLS_DIR}; Caddy loads it via \`tls\`
# and performs no ACME itself.
{
...
${DOMAIN} {
	...
	# Static assets for the maintenance page (and, later, shared web-content assets),
	...
	# Phase 5 content (with the maintenance page as the fallback when it is down):
	#   handle {
	#       reverse_proxy unix//run/euler/content.sock
	#   }

	# Phase 3: no app upstreams yet, so every non-health, non-asset request is the
	# maintenance holding page. Later phases replace this catch-all with the content
	# reverse_proxy above; handle_errors keeps serving the same page on upstream failure.
	handle {
		import maintenance
	}
