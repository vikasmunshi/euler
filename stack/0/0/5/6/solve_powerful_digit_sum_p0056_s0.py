...
# -*- coding: utf-8 -*-
"""Migrated from
    euler_solver/solutions/solutions_0001_0100/solution_0056/p0056.py :: solve_powerful_digit_sum_p0056_s0.
...
def solve(*, num_digits: int) -> int:
    ...
    if not show_solution():
        ...
        data_matrix = np.zeros(shape=(stop_at - start_at, stop_at - start_at),
                               dtype=np.uint64)  # type: ignore[var-annotated]
        ...
        print_e(data_matrix)
        print_e(f'data_matrix.shape={data_matrix.shape!r}')
        print_e(len_matrix)
        print_e(f'len_matrix.shape={len_matrix.shape!r}')
        print_e(avg_matrix)
        print_e(f'avg_matrix.shape={avg_matrix.shape!r}')
        print_e(f'avg_matrix.max()={avg_matrix.max()!r}')
...
if __name__ == '__main__':
    print(solve(num_digits=int(sys.argv[1])))
