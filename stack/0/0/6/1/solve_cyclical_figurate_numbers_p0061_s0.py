...
# -*- coding: utf-8 -*-
"""Migrated from
    euler_solver/solutions/solutions_0001_0100/solution_0061/p0061.py :: solve_cyclical_figurate_numbers_p0061_s0.
...
p_funcs: dict[FigurateNumber, Callable[[int], int]] = {FigurateNumber.TRIANGLE: nth_triangle_number,
...
c_funcs: dict[FigurateNumber, Callable[[int], tuple[float, int, int]]] = {
    FigurateNumber.TRIANGLE: closest_triangle_number, FigurateNumber.SQUARE: closest_square_number,
    FigurateNumber.PENTAGONAL: closest_pentagonal_number, FigurateNumber.HEXAGONAL: closest_hexagonal_number,
    FigurateNumber.HEPTAGONAL: closest_heptagonal_number, FigurateNumber.OCTAGONAL: closest_octagonal_number}
...
def solve(*, length: int) -> list:
    ...
    valid_cyclic_paths: list[tuple[int, ...]] = sorted((path for path in cyclic_paths
                                                        if verify_polygon_types(path, p_numbers)),
                                                       key=sum)
    if show_solution():
        ...
        for valid_cyclic_path in valid_cyclic_paths:
            ...
            print(f'length={length!r} {figurates}', file=sys.stderr)
            pprint(figurates, stream=sys.stderr)
...
if __name__ == '__main__':
    ...
    print(solve(length=int(sys.argv[1])))
