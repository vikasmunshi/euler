...
from euler.setup.paths import MAX_SHARABLE, get_file_path_structure, get_module_name, get_module_path
...
def module_fqdn(euler_problem: int) -> str:
    return f'euler.{get_file_path_structure(euler_problem, sep=".")}.{get_module_name(euler_problem)}'


def make_stub(source_code: str) -> str:
    return stubber_re.sub(r'\1\n    ...\n\n', source_code)


def get_module(euler_problem: int, force_recreate: bool = False) -> str:
    py_file_path: Path = get_module_path(euler_problem)
    pyi_file_path: Path = py_file_path.with_suffix('.pyi')
    if force_recreate:
        return re_create_module(euler_problem, py_file_path=py_file_path, pyi_file_path=pyi_file_path)
    if not py_file_path.exists():
        with Runner() as runner:
            source_code: str = runner.run(get_new_solution_template(euler_problem, model=AgenticModel.gpt_4_1_mini))
        with FileLock(py_file_path, 'write') as f:
            f.write(source_code)
        logger.info({'action': 'Project Euler Solution Created', 'euler_problem': euler_problem})
    if not pyi_file_path.exists():
        with FileLock(py_file_path, 'read') as f:
            source_code = f.read()
        stub_code = make_stub(source_code)
        with FileLock(pyi_file_path, 'write') as f:
            f.write(stub_code)
        logger.info({'action': 'Project Euler Solution Stub Created', 'euler_problem': euler_problem})
    return module_fqdn(euler_problem)


def re_create_module(euler_problem: int, *, py_file_path: Path, pyi_file_path: Path) -> str:
    ...
    if euler_problem > MAX_SHARABLE:
        py_file_path.rename(py_file_path.with_suffix('.bak'))
    with Runner() as runner:
        new_source_code: str = runner.run(get_new_solution_template(euler_problem, model=AgenticModel.gpt_4_1_mini))
    if source_code:
    ...
    with FileLock(py_file_path, 'write') as f:
        f.write(source_code)
    stub_code: str = make_stub(source_code)
    with FileLock(pyi_file_path, 'write') as f:
        f.write(stub_code)
    return module_fqdn(euler_problem)


def get_test_cases_from_module(euler_problem: int) -> list[dict[str, Any]]:
    try:
        module: ModuleType = import_module(module_fqdn(euler_problem))
...
def record_answer_in_module(euler_problem: int, answer: int | str) -> None:
    with FileLock(get_module_path(euler_problem), 'read') as f:
        source_code: str = f.read()
    source_code = answer_re.sub(f'Answer: {answer}', source_code)
    with FileLock(get_module_path(euler_problem), 'write') as f:
        f.write(source_code)
...
async def get_new_solution_template(euler_problem: int, model: AgenticModel) -> str:
    if not getenv('OPENAI_API_KEY'):
        logger.warning({'action': 'missing openai api key', 'euler problem': euler_problem})
