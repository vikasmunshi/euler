#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r""" Solution to Project Euler problem 84: monopoly_odds
...
Problem Statement:
  ...
  (chance) changes this distribution. In addition to G2J, and one card from each
  of CC and CH, that orders the player to go directly to jail, if a player rolls
  three consecutive doubles, they do not advance the result of their 3rd roll.
  Instead they proceed directly to jail. At the beginning of the game, the CC and
  CH cards are shuffled. When a player lands on CC or CH they take a card from the
  top of the respective pile and, after following the instructions, it is returned
  to the bottom of the pile. There are sixteen cards in each pile, but for the
  purpose of this problem we are only concerned with cards that order a movement;
  any instruction not concerned with movement will be ignored and the player will
  remain on the CC/CH square. Community Chest (2/16 cards): Advance to GO Go to
  JAIL  Chance (10/16 cards): Advance to GO Go to JAIL Go to C1 Go to E3 Go to H2
  Go to R1 Go to next R (railway company) Go to next R Go to next U (utility
  company) Go back 3 squares.  The heart of this problem concerns the likelihood
  of visiting a particular square. That is, the probability of finishing at that
  ...
  out of jail", assuming that they pay to get out on their next turn. By starting
  at GO and numbering the squares sequentially from 00 to 39 we can concatenate
  ...
  102400. If, instead of using two 6-sided dice, two 4-sided dice are used, find
...
test_cases: list[TestCase] = [
    TestCase(
        answer='102400',
        is_main_case=False,
        kwargs={'dice_size': 6, 'simulations': 1000000},
    ...
    TestCase(
        answer='101524',
        is_main_case=True,
        kwargs={'dice_size': 4, 'simulations': 1000000},
        solution_execution_time=None,
        solved=False
    ),
]
...
# Register this function as a solution for problem #84
@register_solution(problem_number=84, test_cases=test_cases)
def monopoly_odds(*, dice_size: int, simulations: int) -> str:
...
if __name__ == '__main__':
    ...
    raise SystemExit(evaluate_solutions(84))
